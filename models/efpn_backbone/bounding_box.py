...
class BoundingBoxGenerator(nn.Module):
    def __init__(self, in_channels, num_classes, num_anchors):
        """
        Parameters:
            in_channels (int): Number of channels/depth of the image
            num_classes (int): Number of classes of the dataset (plus the background)
            num_anchors (int): Number of anchors generated by the Anchor generator
        """
        ...
        self.regression_convolution = nn.Conv2d(256, num_anchors * 4, kernel_size=1) 
...
def encode_bounding_boxes(matched_ground_truth_boxes, anchors):
    """
    Encode ground truth bounding boxes with respect to anchor boxes.
    """
    dx_min = matched_ground_truth_boxes[:, 0] - anchors[:, 0]
    dy_min = matched_ground_truth_boxes[:, 1] - anchors[:, 1]
    dx_max = matched_ground_truth_boxes[:, 2] - anchors[:, 2]
    dy_max = matched_ground_truth_boxes[:, 3] - anchors[:, 3]
