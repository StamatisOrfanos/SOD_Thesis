...
class DetectionLoss(nn.Module):
    ...
    def forward(self, pred_bboxes, pred_scores, gt_bboxes, gt_labels):
        ...
        self.anchors = self.anchors.to(device)  # Ensure anchors are on the correct device
        ...
        regression_targets = regression_targets.unsqueeze(0).repeat(pred_bboxes.size(0), 1, 1)  # Adjust for batch size
        ...
        classification_targets = classification_targets.unsqueeze(0).repeat(pred_scores.size(0), 1)  # Adjust for batch size

        # Flatten the scores and targets to calculate classification loss
        classification_loss = self.classification_loss_fn(
            pred_scores.reshape(-1, pred_scores.size(-1)),  # Use reshape for safe dimension handling
            classification_targets.reshape(-1)  # Flatten to match scores
        )
